  ...
  const settingsTileContent = useMemo(() => {
    return (
      <div className="flex flex-col gap-4 h-full w-full items-start overflow-y-auto bg-white ">
        ...
        <ConfigurationPanelItem title="Manager Details">
          <div className="flex flex-col gap-2">
            {metadata?.map((data, index) => (
              <NameValueRow
                ...
                name={data.name}
                value={data.value}
              valueColor={ `${themeColor}-500`}
              />
            ))}
          </div>
        </ConfigurationPanelItem>
        <ConfigurationPanelItem title="SynergyAI Status">
          <div className="flex flex-col gap-2">
            <NameValueRow
              name="SynergyAI connected"
              value={
